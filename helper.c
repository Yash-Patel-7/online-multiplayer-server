...
char** strTokenize(const char *str, const char *delimiters, size_t *numOfTokens, const char *specialTokens) {
    ...
    // use function called "strchr" to check if any character in specialTokens is also in delimiters
    ...
    for (size_t i = 0; i < newStrLen; i++) {
        ...
        if (strchr(specialTokens, newStr[i]) != NULL) {
            ...
            if (tokens[tokenIndex] == NULL) {
                Free(newStr);
                freeArrayOfStrings(tokens, tokenIndex);
                return NULL;
            }
...
void printStrTokens(char **tokens, size_t numOfTokens, const char *delimiter) {
    ...
    for (size_t i = 0; i < numOfTokens; i++) {
        printf("%s", tokens[i]);
        // if index i is not the last token, then print delimiter
        if (i != numOfTokens - 1) {
            printf("%s", delimiter);
        }
    }
...
char* strCombineTokens(char **tokens, size_t numOfTokens, const char *delimiter) {
    ...
    for (size_t i = 0; i < numOfTokens; i++) {
        ...
        // if index i is not the last token, then add delimiter
        if (i != numOfTokens - 1) {
...
// function that reads a line from a file and returns the contents as a string
// returns NULL if there is an error or if the end of the file is reached
char* read_file(int fd) {
    // if fd is invalid, then return NULL
    if (fd < 0) {
        return NULL;
    }
    ...
    if (readStatus == 0) {
        ...
        // if buffer is empty, then return NULL
        if (strlen(buffer) == 0) {
            Free(buffer);
            return NULL;
        }
